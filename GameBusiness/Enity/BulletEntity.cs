...
public struct BulletEntity {
    ...
    public int typeID; //1=w玩家2子弹模式  2==玩家三子弹模式 3==飞行敌人子弹 4==固定敌人子弹
    public sbyte ally; //1=player 2=enemy
    public Texture2D texture2D;
    public float ppu;

    // ===移动===
    public void Move(Vector2 dir, float dt) {   // 玩家2发子弹模式
        pos += Raymath.Vector2Normalize(dir) * moveSpeed * dt;
    }
    // public void PlayerBulMove3(Vector2 dir,float dt)
    // {
    ...
    public void Draw(float scale) {
        ...
        Rectangle dest = new Rectangle(pos.X, pos.Y, scale * ppu * texture2D.Width / 6, scale * ppu * texture2D.Height / 6);
        Vector2 center = new Vector2(scale * ppu * texture2D.Width / 6 / 2, scale * ppu * texture2D.Width / 6 / 2);
